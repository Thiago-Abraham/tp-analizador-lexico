...
void agregarNodoALista(
    NodoToken** lista, const char* lexema, TipoToken tipo,
    TipoDeLista tipoLista, int linea,
    int columna) {  // Doble puntero en C es pasaje por referencia en C++
  if (*lista == NULL) {
...
void imprimirLista(NodoToken* lista, TipoDeLista tipo) {
  ...
  switch (tipo) {
    case tipoListaIdentificadores:
      while (actual != NULL) {
        printf("%s: aparece %d %s\n", actual->lexema, actual->contador,
               actual->contador == 1 ? "vez" : "veces");
        actual = actual->siguiente;
      }
      break;
    ...
    case tipoListaConstantesDecimales:
      while (actual != NULL) {
        printf("%s: valor %d\n", actual->lexema,
               (int)strtol(actual->lexema, NULL, 10));
        actual = actual->siguiente;
      }
      fprintf(stdout,
              "Total acumulado de sumar todas las constantes decimales: %d\n",
              sumarListaDecimales(lista));
      break;
    case tipoListaConstantesOctales:
      while (actual != NULL) {
        printf("%s: valor entero decimal %lu\n", actual->lexema,
               strtoul(actual->lexema, NULL, 8));
        actual = actual->siguiente;
      }
      break;
...
int compararPorLongitud(NodoToken* a, NodoToken* b) {
  return (int)a->longitud -
         (int)b->longitud;  // ascendente: menor longitud primero
...
int compararAlfabetico(NodoToken* a, NodoToken* b) {
  return strcasecmp(
      a->lexema,
      b->lexema);  // si b va antes alfabeticamente que a devuelve un num > 0
}

// OPTIMIZAMOS PARA QUE ORDENAR UNA LISTA RECIBA LA COMPARACION COMO PARAMETRO
// (corte paradigma funcional)
void ordenarListaPor(NodoToken** lista,
  ...
  do {
    ...
    while (actual->siguiente != ultimo) {
      ...
      // en vez de la comparación hardcodeada, llamamos al puntero funcion de
      // comparacion
...
float retornarParteEntera(NodoToken* Lista) {
  int entero = (int)atof(
      Lista->lexema);  // hace atof al lista lexema lo que hace que funcione
                       // como numero (float en este caso) y lo convierte a int
                       // para quedarse solo con la parte entera
  return (float)entero;  // returnea l parte entera ya obtenida trasnformada en
                         // float,es decir entero.0000.....
}

// Retornar mantisa
float retornarMantisa(NodoToken* Lista) {
  float mantisa = atof(Lista->lexema) -
                  retornarParteEntera(
                      Lista);  // hace atof al lexema para que deje de ser char
                               // y leresta la parte entera transformada en
                               // float usando la funcion anterior
  return mantisa;              // returnea
}
// HAY QUE SEGUIR DESARROLLANDO LAS FUNCIONES PARA CADA CASO, LEER CONSIGNA DE
// TP No hace falta formatear las listas Hexadecimales ni octales
